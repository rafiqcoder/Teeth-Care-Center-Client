...
const UsersList = () => {
    ...
    const { data: users=[],refetch } = useQuery({
        queryKey: ["users"],
        queryFn: async  () => {
            const res = await fetch(`https://teeth-cares-server.vercel.app/users?email=${user.email}`,{
    ...
    const handleDelete = (id) => {
        fetch(`https://teeth-cares-server.vercel.app/users/${id}`,{
            method: 'DELETE',
        })
            .then(res => res.json())
            .then(data => {
  ...
  const handleRoleToAdmin = (id) => {
    fetch(`https://teeth-cares-server.vercel.app/users/admin/${id}?email=${user.email}`, {
      method: "PATCH",
      headers: {
  ...
  const handleRoleToSub = (id) => {
    fetch(`https://teeth-cares-server.vercel.app/users/subscriber/${id}?email=${user.email}`, {
      method: "PATCH",
      headers: {
        authorization: `Barear ${localStorage.getItem("token")}`,
      },
